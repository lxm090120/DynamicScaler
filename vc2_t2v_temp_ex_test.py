...
class RunArgs:
    ...
    base_ckpt_path: str = "./videocrafter_models/base_512_v2/model.ckpt"
    seed: int = 114514
...
def main(run_args: RunArgs, prompt, # image_path, image_folder,
    ...
    with open(os.path.join(output_dir, "info.json"), 'w', encoding='utf-8') as f:
        ...
        info_dict = {
            "prompt": prompt,
            "seed": vargs.seed,
            "frame_len": 16 * vargs.f_window_num
        }
    ...
    basic_sampled_video_frames, basic_sampled_latent = pipeline.basic_sample_shift_multi_windows_with_overlap(
        ...
        total_w=512 * num_windows_w,
        total_h=320 * num_windows_h,
        total_f=16 * num_windows_f,
        ...
        loop_window_from_percentage_w=0,
...
if __name__ == "__main__":
    ...
    prompt_dict = {
        1: "a white sailboat gliding through clear blue ocean, few white clouds drift in sunny sky.",
        2: "white clouds drifting in the sky",
        3: "fireworks in the night sky over the city skyline",
        4: "An astronaut walking on the moon’s surface",
        5: "A bird flapping its wings, flying through a clear blue sky",
        6: "a group of hot air balloons flying over a green field under cloudy blue sky",
    }

    prompt = prompt_dict[vargs.prompt_idx]
    print(f"choosing [{vargs.prompt_idx}]: {prompt}")
    # "a white sailboat gliding through clear blue ocean, few white clouds drift in sunny sky."
    #
    # "a white sailboat gliding through clear blue ocean, few white clouds drift in sunny sky."
    # "a sailboat gliding through blue ocean, white clouds drift in sky."


    # "An astronaut dancing gracefully on the lunar surface beneath a spectacular starlit sky. The Milky Way stretches across the dark space, filled with countless twinkling stars."

    # "Astronaut dancing playfully on the lunar surface against the black void of space."
    #


    # "Medium shot of a traveler walking through a desert, captured from the side"
    # "a traveler walking through a desert landscape under the blazing sun and blue sky"
    # "side view of A person walking alone through a vast desert landscape under the blazing sun. Clear blue sky with intense sunlight casting sharp shadows. Golden sand dunes stretching to the horizon."
    # "An astronaut walking on the moon’s surface, high-quality, 4K resolution"
    # "A bird flapping its wings, flying through a clear blue sky"
    ...
    num_windows_w = 1   # 总长度是 width(512) * num_windows
    num_windows_h = 1
    num_windows_f = vargs.f_window_num
    ...
    docking_w = True           # 阻止 window 跨越边界, 而是改为 docking 在边界, 将会增加 T 次去噪步数
    docking_h = True
    if vargs.looping:
        docking_f = False
    else:
    ...
    # origin_prompt_dict = {
    #     0: "A majestic bald eagle soaring through a clear blue sky, flapping its wings, against a backdrop of snow-capped mountains",
    #     256: "A majestic bald eagle soaring through a clear blue sky, wings spread wide",
    #     768: "A majestic bald eagle soaring through a clear blue sky, flapping its wings, against a backdrop of snow-capped mountains",
    # }

    # origin_prompt_dict = {
    #     0: "Astronaut dancing playfully on the lunar surface against the black void of space.",
    #     256: "Astronaut dancing playfully on the lunar surface against the black void of space.",
    #     768: "Astronaut dancing playfully on the lunar surface against the black void of space.",
    # }
    origin_prompt_dict = None

    per_frame_prompt_dict = None # expand_per_frame_prompts(origin_prompt_dict=origin_prompt_dict, total_frames_num=16*num_windows_f)
    # print(f"per_frame_prompt_dict :{per_frame_prompt_dict}")

    PROJECT_FOLDER = f"T2V_tempEX_eval_{'Loop' if vargs.looping else 'noLoop'}_Fx{vargs.f_window_num}"
    PROJECT_NOTE = f"T2V_SW_temporal-EX-loop-f{vargs.f_window_num}" # -test-merge_overlap_denoise-VAR{merge_renoised_overlap_latent_ratio}-{_merge_prev_step}"
    ...
    #                f"_fps-{run_args.fps}" \
    #                f"_loop-{loop_step}-initloop-{init_stage_loop_step}" \
